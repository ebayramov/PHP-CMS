...
$varr = [
    "sql" => '', "openModal" => '', "action" => "", "lang" => 2, "page" => 0,
    "ctitle" => "", "ctext" => "0", "cimg" => "0",  "caction" => 'cadd', "cbutton" => 'Add',
    "lid" => "", "lshort" => "", "lfull" => "",  "lstatus" => "", "laction" => 'ladd', "lbutton" => 'Add',
    "mid" => "", "morder" => "", "mname" => "", "mstatus" => "", "maction" => 'madd', "mbutton" => 'Add'
];
...
if ($action == "ldel") {
    ...
    header("Location: index.php");
} elseif ($action == "ccheck") {
    $sql = "SELECT `id` FROM `content` WHERE `menu`=$page";
    ...
    $picture = $file ?? "";
    $sql = "INSERT INTO `content` (`menu`, `title`, `text`, `image`) VALUES ($page, '$ctitle', '$ctext', '$picture')";
    header("Location: index.php");
} elseif ($action == "cupdate") {
    $picture = $file ?? "";
    $sql = "UPDATE `content` SET `title`='$ctitle', `text`='$ctext', `image`='$picture'  WHERE `menu`=$page";
    header("Location: index.php");
...
<body>
    <div>
        <header>
            ...
            <div>Welcome <?= $_SESSION["user"] ?><a href="?action=logout"> Log out</a></div>
        </header>
        <main>
            <aside>
                <table>
                ...
                <table>
                    ...
                    while ($row = mysqli_fetch_assoc($result)) {
                        echo '
                                    <tr>
                                        <td>' . $row["order"] . '</td>
                                        <td><a href="?lang=' . $lang . '&action=ccheck&page=' . $row["id"] . '">' . $row["name"] . '</a></td>
                                        <td>
                                            <a href="?lang=' . $lang . '&action=mcheck&mid=' . $row["id"] . '&mstatus=' . $row["status"] . '"><i class="far fa' . ($row["status"] ? '-check' : '') . '-square"></i></a>
                                            <a href="?lang=' . $lang . '&action=medit&mid=' . $row["id"] . '&morder=' . $row["order"] . '&mname=' . $row["name"] . '"><i class="fas fa-edit"></i></a>
                                            <a href="?lang=' . $lang . '&action=mdel&mid=' . $row["id"] . '"><i class="far fa-window-close"></i></a>
                                        </td>
                                    </tr>
                                ';
                    }
            ...
            <section>
                ...
                if ($page == 0) echo '<h3>Bir sehife secin</h3>';
                    ...
                    echo '
                                ...
                                ';
                    echo $img ? '<img src="' . $img . '" style="width: 50px" />' : '';
                    echo '
                                    <p><input type="file" name="cimg" /></p>
                                    <p><textarea name="ctext" >' . $text . '</textarea></p>
                                    <p><input type="submit" name="submit" value="' . $cbutton . '" /></p>
                                </form>
                            ';
                }
                ?>
